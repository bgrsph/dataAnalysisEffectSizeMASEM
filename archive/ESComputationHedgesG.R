...
ESDataESConverted <- mutate (ESDataCBTStudies,
          
                   #Compute Hedges'g
                   hedges_g = (1 - (3 / (4 * (n_t + n_c) - 9))) * ((mean_t - mean_c) / (sqrt(((SD_t^2) * (n_t - 1) + (SD_c^2) * (n_c - 1)) / (n_t + n_c - 2)))),
                   
                   #Convert Hedges' g into r using Aaron et al. formula. If needed values are NA, use the reported r (which is the case for continous bivariate relationship M-Y)
                   rpb_converted = ifelse((is.na(hedges_g)), r, hedges_g / (sqrt(hedges_g^2 + 4 - (8/(n_t + n_c))))),
              
                   #Reverse score rpb_converted that are comprised of 1 variable that is in the opposite direction, otherwise same direction
...
#4- Based on the code of meta-analysis (see ESComputation.R), reverse r if one of the conditions below applies
...
ESDataSignsReversed <- mutate (ESDataESConverted,
...
#5- Aggregate sample sizes and correlations when multiple relationships within same study have multiple effect sizes and/or sample sizes.
webMASEMHedgesG2 <- ESDataSignsReversed %>% # Re-code variables for webMASEM-compatible formatting
  mutate(
    M_X = ifelse(var1type == "1" & var2type == "MA", rpb_converted, NA), # M_X: negativeCognition_CBT
    Y_M = ifelse(var1type == "MA" & var2type == "O", rpb_converted, NA), # Y_M: depression_negativeCognition
    Y_X = ifelse(var1type == "1" & var2type == "O", rpb_converted, NA)   # Y_X: depression_CBT
  ) %>%
  group_by(studyid) %>% #
  summarise(
...
# Write the dataframe into csv format to be inputted in WebMASEM.
write.csv(webMASEMHedgesG2, "~/Desktop/repo/dataAnalysisEffectSizeMASEM/output/webMASEMHedgesG.csv")